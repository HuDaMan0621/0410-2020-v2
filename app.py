...
# Load the main music
theme_song = pyglet.media.load('./resources/ff7.wav')
music = pyglet.media.Player()
music.queue(theme_song)

# Set up the two top labels
score_label = pyglet.text.Label(text="Caught 0", x=10, y=75, batch=main_batch)
life_label = pyglet.text.Label(text="Life", x=10, y=25, batch=main_batch)
...
goblin2 = monster.Monster(x=randint(0, WIDTH), y=randint(0,HEIGHT), batch=main_batch)

# Store all objects that update each frame in a list
game_objects = [hero, goblin, goblin2]
...
def game_over():
    ...
    #     print('do you want to keep play ? Y/N')
    #     user_input = input()
    #     if user_input == ('Y'):
    ...
    pyglet.text.Label(text="YoU WIN", font_size = 36,  x=100, y=100, batch=main_batch)
...
def update(dt):
    ...
    if is_drawing:
        ...
        for to_remove in [obj for obj in game_objects if obj.dead]:
            ...
            gotcha_sound_effect = pyglet.media.load('./resources/Satellite noise.wav', streaming=False)
            gotcha_sound_effect.play()
